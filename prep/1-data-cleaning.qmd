...
```{r}
vpd_30 <- vpd_data |> 
    left_join(fire_info |> dplyr::select(GID, start_date), by = "GID") |> 
    mutate(
        date = as.Date(date),
...
vpd_4 <- vpd_data |> 
    ...
    filter(date >= (start_date - 30) & date < start_date) |>  # keep 1–30 days before start
    # calculate the mean vpd for each week
    mutate(week = case_when(
        date >= (start_date - 7) & date < start_date ~ 1,
        date >= (start_date - 14) & date < (start_date - 7) ~ 2,
        date >= (start_date - 21) & date < (start_date - 14) ~ 3,
        date >= (start_date - 30) & date < (start_date - 21) ~ 4,
        TRUE ~ NA_real_
    )) |>    group_by(GID, week) |>
    summarize(vpd = mean(vpd, na.rm = TRUE), .groups = "drop") |>
    # make lag 1-4 based on week
    mutate(lag = as.integer(week)) |> 
    dplyr::select(-week)
```
...
vpd_wide_4_gids = unique(vpd_wide_4$GID)
nrow(vpd_wide_4)

# save as vpd wide 4
saveRDS(vpd_wide_4, here("data", "cleaned", "vpd_4_wide.rds"))
```
...
```
 ...
 ```{r}
rbr_data = read_csv(here("data", "batch_rbr_w_offset_zonal_stats_p95.csv")) |> 
    select(GID, median, percentile_95)  |> 
    rename(pct_95 = percentile_95) 
...
```{r}
cmi_data = read_csv(here("data", "cmi_water_year_anomalies.csv"))
...
```{r}
saveRDS(cmi_wide, "../data/cleaned/cmi_wy_cleaned_20260519.rds")
```


# save cmi_ma
```{r}
saveRDS(cmi_ma, here("data", "cleaned","cmi_wy_ma_5_20260519.rds"))
```
