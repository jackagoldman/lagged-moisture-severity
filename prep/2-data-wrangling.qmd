...
cmi_wide <- readRDS(here("data", "cleaned","cmi_wy_cleaned_20260519.rds"))
```
...
moisture_final <- fwi |> 
    ...
    left_join(landcover, by = "GID")  |> drop_na() |> 
    arrange(GID)
...
```{r}
duplicate_gids <- moisture_final %>% 
  group_by(GID) %>% 
  filter(n() > 1) %>% 
  ungroup() %>% 
  distinct()
nrow(duplicate_gids) # 9 duplicate GIDs
```

# remove duplicate GIDs keeping the first occurrence
```{r}
moisture_final <- moisture_final %>% 
  distinct(GID, .keep_all = TRUE)
nrow(moisture_final) # should be 1088 now
```
...
# find rows in moisture_final that are not in vpd_wide GIDs 

```{r}
missing_vpd_gids <- setdiff(moisture_final$GID, vpd_wide$GID)
length(missing_vpd_gids) # 9 missing GIDs
missing_vpd_gids
```
...
vpd_ma <- vpd_ma %>% filter(GID %in% moisture_df$GID)
moisture_df <- moisture_df %>% 
    left_join(vpd_ma %>% select(GID, vpd_ma_30), by = "GID")
# scale using function
```

```{r}
saveRDS(moisture_df, here("data", "analysis_ready", "moisture_lagged_non_scaled_data_20260519.rds"))
