...
vpd_wide4 <- readRDS(here("data", "cleaned","vpd_4_wide.rds"))
...
fwi <- read_csv(here("data", 'fwi_95th_duration_by_fire_full.csv'))
...
vpd_wide4 <- round_columns(vpd_wide4)
...
vpd_wide4 <- scale_columns(vpd_wide4, colnames(vpd_wide4)[-1])
...
# remove rows in vpd wide 4

```{r}
vpd_wide4 <- vpd_wide4 %>% filter(GID %in% moisture_final$GID)
nrow(vpd_wide4) # now matches moisture_final
```
...
```{r}
vpd_wide4 <- vpd_wide4 %>% arrange(GID) # make sure the GIDs are in the same order
vpd_matrix4 <- as.matrix(vpd_wide4 %>% select(-GID))
head(vpd_matrix4)
```
...
vpd4_gids = unique(vpd_wide4$GID)
length(vpd_gids)
...
moisture_final <- moisture_final %>% filter(GID %in% vpd_gids | GID %in% cmi_gids | GID %in% vpd4_gids)
nrow(moisture_final) #
...
n <- nrow(vpd_matrix4)
L4 <- ncol(vpd_matrix4)
# create matrix L4 based on nrow and ncol of vpd_matrix4 where each column value is the lag index
L4 <- matrix(NA, nrow = n, ncol = L4)
for (i in 1:ncol(L4)) {
  L4[, i] = i
}
head(L4)
...
```{r}
levels(moisture_df$ecoregion_lvl)
weights_acp4 <- weights_btl4 <- weights_ln4 <- weights_lsj4 <- weights_lw4 <- weights_pr4 <- matrix(1, ncol = ncol(moisture_df$L4), nrow = nrow(moisture_df$L4))
weights_acp4[!(moisture_df$ecoregion_lvl == 'ACP'), ] <- 0
weights_btl4[!(moisture_df$ecoregion_lvl == 'BTL'), ] <- 0
weights_ln4[!(moisture_df$ecoregion_lvl == 'LN'), ] <- 0
weights_lsj4[!(moisture_df$ecoregion_lvl == 'LSJ'), ] <- 0
weights_lw4[!(moisture_df$ecoregion_lvl == 'LW'), ] <- 0
weights_pr4[!(moisture_df$ecoregion_lvl == 'PR'), ] <- 0
#Adding these weighting matrices to the data list gives us all we need to set up group-level or hierarchical distributed lag terms(vpd4)
moisture_df$weights_acp4 <- weights_acp4
moisture_df$weights_btl4 <- weights_btl4
moisture_df$weights_ln4 <- weights_ln4
moisture_df$weights_lsj4 <- weights_lsj4
moisture_df$weights_lw4 <- weights_lw4
moisture_df$weights_pr4 <- weights_pr4
```
...
```{r}
saveRDS(moisture_df, here("data", "analysis_ready", "moisture_lagged_non_scaled_data.rds"))
